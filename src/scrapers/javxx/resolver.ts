...
export async function resolveSourceUrl(filename: string): Promise<string | null> {
  ...
  try {
    ...
    const searchHtml = await page.evaluate('document.getElementById("app")?.innerHTML || ""');
    if (!searchHtml) {
