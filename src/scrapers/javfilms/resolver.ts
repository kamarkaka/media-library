...
export async function resolveSourceUrl(filename: string): Promise<string | null> {
  ...
  const searchUrl = SEARCH_URL_PREFIX + '?keywords=' + searchCode;
  ...
  try {
    ...
    const searchHtml = await page.evaluate('document.getElementById("maincontainer")?.innerHTML || ""');
    if (!searchHtml) {
    ...
    $('.container .videobox a').each((_, el) => {
