...
export async function resolveSourceUrl(filename: string): Promise<string | null> {
  ...
  try {
    ...
    const searchHtml = await page.evaluate('document.getElementsByClassName("content-without-search")[0]?.innerHTML || ""');
    if (!searchHtml) {
    ...
    $('div.grid .thumbnail.group a.text-secondary').each((_, el) => {
